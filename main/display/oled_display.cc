...
void OledDisplay::HideDefaultUI(bool hide) {
    // Hide or show content UI elements (but keep status bar, container, and content visible)
    if (hide) {
        // Keep status_bar_, container_, and content_ visible
        // Only hide the emotion icon and chat message labels
        if (content_left_) lv_obj_add_flag(content_left_, LV_OBJ_FLAG_HIDDEN);
        ...
        if (side_bar_) lv_obj_add_flag(side_bar_, LV_OBJ_FLAG_HIDDEN);
    } else {
        // Restore everything
        if (content_left_) lv_obj_clear_flag(content_left_, LV_OBJ_FLAG_HIDDEN);
        if (content_right_) lv_obj_clear_flag(content_right_, LV_OBJ_FLAG_HIDDEN);
        if (emotion_label_) lv_obj_clear_flag(emotion_label_, LV_OBJ_FLAG_HIDDEN);
        if (chat_message_label_) lv_obj_clear_flag(chat_message_label_, LV_OBJ_FLAG_HIDDEN);
        if (side_bar_) lv_obj_clear_flag(side_bar_, LV_OBJ_FLAG_HIDDEN);
    }
}
